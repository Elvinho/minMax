...
function ckmouse(e) {
	...
	div.innerHTML = "Adicionar Nó Filho";
...
function editar (pkNodo) {
	...
	if (nodo == null) {
		alert("O nó não existe.");
		return false;
	}
	var valor = prompt("Type a number:");
	
	if (!isNumber(valor)) {
		alert("Este valor não é um número.");
		return;
	}
	nodo.valor = valor;
...
function folhasPreenchidas() {
	for (var i = 0; i < nodos.length; i++) {
		...
		if (nodo.filhos.isEmpty() && nodo.valor == null) {
			alert("Você não inseriu nós suficientes, insira mais nós!");
			return false;
		}
	}
	return true;
...
function excluirNodo(pkNodo, alertar) {
	if (pkNodo == 1) {
		alert("Não é possível deletar o nó raiz!")
		return;
	}

	var nodo = findByPK(pkNodo);
	if (nodo == null) {
		alert("Este nó não existe!.");
		return false;
	}
...
for (let i = 15; i <= 30; i++) {
    ...
    do {
        valorAleatorio = Math.floor(Math.random() * 100) + 1;
    } while (valoresUnicos.has(valorAleatorio));

    nodos[i].valor = valorAleatorio;
